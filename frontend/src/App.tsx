...
export default function App() {
  ...
  const loanAmount = formData.property_price - formData.down_payment;
  ...
  const handleDeleteLoan = async () => {
    ...
    if (!window.confirm(`Вы уверены, что хотите удалить кредит "${formData.name}"? Это действие необратимо.`)) {
      return;
    }
  ...
  const handleTogglePaidMonth = async (paymentNumber: number, isPaid: boolean) => {
    if (!currentLoanId) return;
    try {
      if (isPaid) {
        await axios.delete(`${API_URL}/loans/${currentLoanId}/paid-months/${paymentNumber}`);
      } else {
        await axios.post(`${API_URL}/loans/${currentLoanId}/paid-months/${paymentNumber}`);
      }
      await loadLoanData(currentLoanId);
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex space-x-2 overflow-x-auto pb-2 custom-scrollbar">
          ...
          <button
            ...
            className={`flex items-center space-x-1 px-4 py-2 rounded-lg whitespace-nowrap text-sm font-medium transition-all duration-200 border border-dashed ${
              !currentLoanId && loansList.length > 0
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 p-6 rounded-2xl shadow-xl border border-slate-800 h-fit sticky top-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-white font-medium"
                    required
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Стоимость недвижимости (₽)</label>
                <div className="relative">
                  <Home className="absolute left-3 top-2.5 text-slate-500" size={18} />
                  <input 
                    type="text" 
                    value={formatNumber(formData.property_price)}
                    onChange={(e) => setFormData({...formData, property_price: parseNumber(e.target.value)})}
                    className="w-full pl-10 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-white font-medium"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Первоначальный взнос (₽)</label>
                <div className="relative">
                  <Wallet className="absolute left-3 top-2.5 text-slate-500" size={18} />
                  <input 
                    type="text" 
                    value={formatNumber(formData.down_payment)}
                    onChange={(e) => setFormData({...formData, down_payment: parseNumber(e.target.value)})}
                    className="w-full pl-10 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-white font-medium"
                  />
                </div>
                <div className="mt-2 text-sm bg-slate-800/50 p-2 rounded border border-slate-700/50 text-slate-400 flex justify-between">
                  <span>Сумма кредита:</span>
                  <span className="font-semibold text-white">{formatMoney(loanAmount)}</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-1">Ставка (%)</label>
                  <div className="relative">
                    <Percent className="absolute left-3 top-2.5 text-slate-500" size={18} />
                    <input 
                      type="number" step="0.1"
                      value={formData.interest_rate}
                      onChange={(e) => setFormData({...formData, interest_rate: Number(e.target.value)})}
                      className="w-full pl-10 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-white"
                    />
                  </div>
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-400 mb-1">Срок (лет)</label>
                  <div className="relative">
                    <Calendar className="absolute left-3 top-2.5 text-slate-500" size={18} />
                    <input 
                      type="number" 
                      value={formData.term_years}
                      onChange={(e) => setFormData({...formData, term_years: Number(e.target.value)})}
                      className="w-full pl-10 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-white"
                    />
                  </div>
                </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-400 mb-1">Тип платежа</label>
                <select 
                  value={formData.payment_type}
                  onChange={(e) => setFormData({...formData, payment_type: e.target.value})}
                  className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-white"
                >
              ...
              <button 
                type="submit" 
                disabled={loading}
                className={`w-full text-white font-medium py-3 rounded-lg transition-colors mt-4 shadow-lg disabled:opacity-50 ${
                  currentLoanId 
                  ? 'bg-slate-700 hover:bg-slate-600 shadow-none border border-slate-600' 
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <div className="bg-slate-900 p-5 rounded-2xl shadow-lg border border-slate-800">
                    ...
                    <div className="text-2xl font-bold text-white">
                      {formatMoney(loanAmount + scheduleData.total_interest)}
                    </div>
                  </div>
                </div>
                  ...
                  <form onSubmit={handleAddExtraPayment} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                      <div>
                        ...
                        <input 
                          type="date" 
                          value={extraPayment.payment_date}
                          onChange={(e) => setExtraPayment({...extraPayment, payment_date: e.target.value})}
                          className="w-full px-3 py-2 bg-slate-950 border border-indigo-900/50 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-white"
                        />
                      </div>
                      <div>
                        ...
                        <div className="relative">
                           ...
                           <input 
                            type="text" 
                            value={formatNumber(extraPayment.amount)}
                            onChange={(e) => setExtraPayment({...extraPayment, amount: parseNumber(e.target.value)})}
                            className="w-full pl-8 pr-3 py-2 bg-slate-950 border border-indigo-900/50 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm font-medium text-white"
                          />
                        </div>
                      </div>
                      <div>
                        ...
                        <select 
                          value={extraPayment.strategy}
                          onChange={(e) => setExtraPayment({...extraPayment, strategy: e.target.value})}
                          className="w-full px-3 py-2 bg-slate-950 border border-indigo-900/50 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-sm text-white"
                        >
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-indigo-900/30">
                      <label className="flex items-center space-x-2 cursor-pointer">
                        <input 
                          type="checkbox" 
                          checked={extraPayment.is_recurring}
                          onChange={(e) => setExtraPayment({...extraPayment, is_recurring: e.target.checked})}
                          className="w-4 h-4 text-indigo-500 bg-slate-950 border-slate-700 rounded focus:ring-indigo-500 focus:ring-offset-slate-900"
                        />
                      ...
                      <button 
                        ...
                        className="bg-indigo-600 hover:bg-indigo-500 text-white font-medium py-2 px-6 rounded-lg transition-colors text-sm shadow-lg shadow-indigo-900/20 disabled:bg-indigo-900"
                      >
                      ...
                      <div className="space-y-2">
                        {activeExtraPayments.map((ep: any) => (
                          <div key={ep.id} className="flex items-center justify-between bg-slate-950 border border-indigo-900/30 rounded-lg p-3 group">
                            <div className="flex items-center space-x-4">
                              <div className="text-sm text-slate-300 font-medium w-24">
                                {new Date(ep.payment_date).toLocaleDateString('ru-RU')}
                              </div>
                              <div className="text-sm font-bold text-emerald-400">
                                {formatMoney(Number(ep.amount))}
                              </div>
                            ...
                            <button 
                              onClick={() => handleDeleteExtraPayment(ep.id)}
                    ...
                    <AreaChart data={scheduleData.schedule}>
                      ...
                      <Tooltip 
                        formatter={(value: any, name: any) => {
                          const valStr = formatMoney(Number(value));
                          const nameStr = name === 'remaining_balance' ? 'Текущий долг' : 'Базовый сценарий';
                    ...
                    <table className="w-full text-left text-sm whitespace-nowrap">
                      ...
                      <tbody className="divide-y divide-slate-800/50">
                        {scheduleData.schedule
                          .filter((row: any) => row.total_payment > 0)
                          .map((row: any) => {
                          ...
                          return (
                            <tr key={row.payment_number} className={`hover:bg-slate-800/50 transition-colors ${isPaid ? 'bg-emerald-950/20 opacity-75' : row.extra_payment > 0 ? 'bg-indigo-900/10' : ''}`}>
                              <td className="p-4 text-center">
                                <button 
                                  onClick={() => handleTogglePaidMonth(row.payment_number, isPaid)}
                                  className={`p-1 rounded-lg transition-colors ${isPaid ? 'text-emerald-400 bg-emerald-900/40' : 'text-slate-600 hover:text-slate-400'}`}
                                >
